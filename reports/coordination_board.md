...
| Sector3D Solve | agent/sector3d-solve | excitation/setup/reports/solve | blocked | 8b31b49 | latest solve-side patch now uses radial terminal faces and PyAEDT save fallback, but the in-AEDT host currently reports `active_project = null` / `project_list = []` and fails before queued scripts reach the excitation body |
