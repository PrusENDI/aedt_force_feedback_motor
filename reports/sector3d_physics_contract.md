...
- current geometry scaffold must build the SSDR calibration model directly as a true periodic sector
...
- full-annulus baseline geometry is no longer acceptable for the production scaffold artifact
...
- a true periodic sector generated by the scaffold, not a full-annulus helper
...
The current geometry scaffold is expected to emit:

- `sector_geometry.geometry_scope = periodic_sector`
- generated radial cut sheets named `Auto3D_Periodic_Master` and `Auto3D_Periodic_Slave`
- a separated rigid support body and double-sided flat-copper phase-belt conductors
- segmented axial magnet poles only within the active SSDR sector
- an expanded air region for coreless fringing/leakage review
